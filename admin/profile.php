...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['update']))
{
    ...
    if (!empty($_FILES['photo']['name']))
    {
        ...
        {
            setFlash('error','Photo upload failed. Use JPG/PNG under 5MB.');
        }
    }
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['change_password']))
{
    ...
    if (!password_verify($old,$dbPass)) setFlash('error','Current password is wrong.');
    elseif ($new!==$confirm) setFlash('error','New passwords do not match.');
    elseif (strlen($new)<6) setFlash('error','Password must be at least 6 characters.');
    else
    {
...
$profile=$conn->query("SELECT id,name,email,phone,profile_photo,created_at FROM users WHERE id={$u['id']} LIMIT 1")->fetch_assoc();
...
<div class="row g-3">
  <div class="col-lg-4">
    <div class="card text-center mb-3">
      <div class="card-body py-4">
        <img src="<?=$profile['profile_photo']==='default.png' ? BASE_URL.'/assets/img/default.png' : UPLOAD_URL.'avatars/'.$profile['profile_photo']?>" class="rounded-circle mb-3" width="90" height="90" style="object-fit:cover;border:3px solid #FFC107;" onerror="this.src='<?=BASE_URL?>/assets/img/default.png'">
        <h5 class="mb-0"><?=htmlspecialchars($profile['name'])?></h5>
        <div class="text-muted" style="font-size:.82rem;"><?=htmlspecialchars($profile['email'])?></div>
        <div class="badge bg-warning text-dark mt-2">Administrator</div>
      </div>
    </div>
    <div class="card">
      <div class="card-header"><i class="bi bi-lock text-warning me-2"></i>Change Password</div>
      <div class="card-body">
        <form method="POST">
  ...
  <div class="col-lg-8">
    <div class="card">
      <div class="card-header"><i class="bi bi-person-gear text-warning me-2"></i>Edit Profile</div>
      <div class="card-body">
        <form method="POST" enctype="multipart/form-data">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6"><label class="form-label">Admin Since</label><input type="text" class="form-control" value="<?=fmtDate($profile['created_at'])?>" readonly></div>
            ...
            <div class="col-12"><button class="btn btn-warning"><i class="bi bi-save me-1"></i>Save Changes</button></div>
          </div>
        </form>
      </div>
    </div>
